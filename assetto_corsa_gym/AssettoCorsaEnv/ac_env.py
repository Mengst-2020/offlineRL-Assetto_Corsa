...
from AssettoCorsaEnv.reference_lap import ReferenceLap,ReferenceTrack
...
try:
    from mpc_assetto_corsa import AssettoCorsaMPCController, DEFAULT_CURVILINEAR_VEHICLE_PARAMS
except ImportError:
    AssettoCorsaMPCController = None
    DEFAULT_CURVILINEAR_VEHICLE_PARAMS = {}
...
# CURV_LOOK_AHEAD_DISTANCE = 500. # METERS####################################################
# CURV_LOOK_AHEAD_VECTOR_SIZE = 25 # SIZE OF THE VECTOR
...
class AssettoCorsaEnv(Env, gym_utils.EzPickle):
    ...
    def __init__(self, config=None,
        ...
        if config == None:
            ...
            import copy
            config = OmegaConf.load("F:/code/assetto_corsa_gym-main/config.yml")
            config = copy.deepcopy(config.AssettoCorsa)
        ...
        if output_path == None:    
            output_path = "rayoutput"
        if ac_configs_path == None:    
            ac_configs_path = "F:/code/assetto_corsa_gym-main/assetto_corsa_gym/AssettoCorsaConfigs"
        ...
        self.lapdist_fixed = np.full(self.max_episode_steps, -1, dtype=np.float32)##############
        self.end_flag=False
        ...
        self.reset_lapdist_points = list(range(220, 5650, 220))
        # 第一次直接到是 0  后面每次先跑 2 圈再到目标点结束
        self.reset_required_laps = [0] + [2] * (len(self.reset_lapdist_points) - 1)
        self.reset_phase_idx = 0
        self.laps_since_reset = 0
        self.prev_lapdist_raw = None
        self.reset_schedule_loop = True
        ...
        self.read_wheel=True
        self.enable_mpc = getattr(self.config, "enable_mpc", False)
        self.mpc_horizon = getattr(self.config, "mpc_horizon", 15)
        self.mpc_dt = getattr(self.config, "mpc_dt", self.dt)
        self.mpc_vx_min = getattr(self.config, "mpc_vx_min", 5.0)
        self.mpc_solver = getattr(self.config, "mpc_solver", "slsqp")
        self.mpc_warm_start = None
        self.mpc_last_action = np.array([0.0, -1.0, -1.0], dtype=np.float32)
        self.mpc_weights = {
            "ey": 20.0,
            "epsi": 15.0,
            "vx": 1.0,
        ...
        self.track_left = ReferenceTrack(self.track_file, use_target_speed=self.use_target_speed,side=1)
        self.track_right = ReferenceTrack(self.track_file, use_target_speed=self.use_target_speed,side=0)

        res=self.ref_lap.classify_corners_from_curvature()
        self.lapdist_buffer = []
        self.lapdist_label= []
        self.failmode=None
        
        self.racing_line = self.ref_lap.get_racing_line_time()
        self.racing_left = self.track_left.get_racing_line_time()
        self.racing_right = self.track_right.get_racing_line_time()

        if torch_device:
            self.racing_line_torch =  torch.tensor(self.racing_line, device=torch_device, dtype=torch.float)
            self.racing_left_torch =  torch.tensor(self.racing_left, device=torch_device, dtype=torch.float)
            self.racing_right_torch =  torch.tensor(self.racing_right, device=torch_device, dtype=torch.float)
        ...
        self.ref_position_x_dist = self.ref_lap.get_channel_dist("position.x").reshape(-1).astype(np.float64)
        self.ref_position_y_dist = self.ref_lap.get_channel_dist("position.y").reshape(-1).astype(np.float64)
        self.ref_yaw_dist = self.ref_lap.get_channel_dist("yaw").reshape(-1).astype(np.float64)
        self.ref_curvature_dist = self.ref_lap.get_channel_dist("curvature").reshape(-1).astype(np.float64)
        if self.use_target_speed:
            self.ref_target_speed_dist = self.ref_lap.get_channel_dist("target_speed").reshape(-1).astype(np.float64)
        else:
            self.ref_target_speed_dist = None
        ...
        self.controls_min_values = np.array([-1.0,  -1.0,  -1.0], dtype=np.float32)
        self.controls_max_values = np.array([ 1.0,   1.0,   1.0], dtype=np.float32)
        ...
        state_low = np.full((self.state_dim,), -1e8, dtype=np.float32)
        state_high = np.full((self.state_dim,), 1e8, dtype=np.float32)
        state_space = Box(low=state_low, high=state_high, dtype=np.float32)
        H = self.config.final_image_height
        W = self.config.final_image_width
        C = 3 if self.config.color_mode == "bgr" else 1
        ...
        # box_space = Box(low=-np.inf, high=np.inf, shape=(self.state_dim,), dtype=np.float32)
        # self.observation_space = Dict({"state": box_space})
    ...
    def _advance_reset_phase(self):
        """进入下一个 LapDist 目标阶段"""
        if len(self.reset_lapdist_points) == 0:
            return
        if self.reset_schedule_loop:
            self.reset_phase_idx = (self.reset_phase_idx + 1) % len(self.reset_lapdist_points)
        else:
            self.reset_phase_idx = min(self.reset_phase_idx + 1, len(self.reset_lapdist_points) - 1)

        self.laps_since_reset = 0

    def _check_custom_reset_condition(self, current_lapdist):
        """检查当前是否触发自定义 reset"""
        if len(self.reset_lapdist_points) == 0:
            return False

        target = self.reset_lapdist_points[self.reset_phase_idx]
        required_laps = self.reset_required_laps[self.reset_phase_idx]

        if self.prev_lapdist_raw is None:
            return False

        # 只有在同一圈内从小于 target 变成 >= target 才算真正经过目标点
        crossed_target = (self.prev_lapdist_raw < target) and (current_lapdist >= target)

        return crossed_target and (self.laps_since_reset >= required_laps)


    def update_racing_line(self, x, y):
        self.racing_line = self.ref_lap.get_racing_line_time()
        self.racing_line = self.racing_line[np.sqrt( (self.racing_line[:,0] - x)**2 + (self.racing_line[:,1] - y)**2 ) < RACING_LINE_DIAMETER_CROP]

        self.racing_left = self.track_left.get_racing_line_time()
        self.racing_left = self.racing_left[np.sqrt( (self.racing_left[:,0] - x)**2 + (self.racing_left[:,1] - y)**2 ) < RACING_LINE_DIAMETER_CROP]

        self.racing_right = self.track_right.get_racing_line_time()
        self.racing_right = self.racing_right[np.sqrt( (self.racing_right[:,0] - x)**2 + (self.racing_right[:,1] - y)**2 ) < RACING_LINE_DIAMETER_CROP]
        if self.torch_device:
            self.racing_line_torch = torch.tensor(self.racing_line, device=self.torch_device, dtype=torch.float)
            self.racing_left_torch = torch.tensor(self.racing_left, device=self.torch_device, dtype=torch.float)
            self.racing_right_torch = torch.tensor(self.racing_right, device=self.torch_device, dtype=torch.float)

    def _load_vehicle_params(self):
        default_params = DEFAULT_CURVILINEAR_VEHICLE_PARAMS.get(self.car_name)
        ...
        config_params = getattr(self.config, "mpc_vehicle_params", None)
        if config_params:
            params.update(dict(config_params))
        return params
    ...
    def _build_mpc_controller(self):
        if AssettoCorsaMPCController is None:
            raise ImportError("mpc_assetto_corsa.py could not be imported")
        if self.mpc_controller is None:
            self.mpc_controller = AssettoCorsaMPCController(
                env=self,
                horizon=self.mpc_horizon,
                dt=self.mpc_dt,
                vehicle_params=self.vehicle_params,
            )
            self.mpc_controller.weights.update(self.mpc_weights)
        return self.mpc_controller

    def get_curvilinear_state(self, state=None):
        if state is None:
            state = self.state

        steer_max_rad = np.deg2rad(self.vehicle_params["steer_max_deg"])
        current_delta = np.clip(self.current_actions[0], -1.0, 1.0) * steer_max_rad
        current_ax = 0.0
        if self.current_actions[1] > -1.0:
            throttle = np.clip((self.current_actions[1] + 1.0) * 0.5, 0.0, 1.0)
            current_ax += throttle * self.vehicle_params["ax_max"]
        if self.current_actions[2] > -1.0:
            brake = np.clip((self.current_actions[2] + 1.0) * 0.5, 0.0, 1.0)
            current_ax -= brake * abs(self.vehicle_params["ax_min"])

        return {
            "s": float(state["s_mpc"]),
            "ey": float(state["ey_mpc"]),
            "epsi": float(state["epsi_mpc"]),
            "vx": float(max(state["vx_mpc"], 0.0)),
            "vy": float(state["vy_mpc"]),
            "r": float(state["r_mpc"]),
            "kappa_ref": float(state["kappa_ref_mpc"]),
            "x": float(state["world_position_x"]),
            "y": float(state["world_position_y"]),
            "yaw": float(state["yaw"]),
            "rl_index": int(state["rl_point"]),
            "ey_min": float(-abs(state["gap_left"])),
            "ey_max": float(abs(state["gap_right"])),
            "u_prev": np.array([current_delta, current_ax], dtype=np.float64),
        }

    def get_mpc_reference(self, s0, horizon=None):
        horizon = horizon or self.mpc_horizon
        kappa_ref = np.zeros(horizon, dtype=np.float64)
        vx_ref = np.zeros(horizon, dtype=np.float64)
        ay_max = float(self.vehicle_params.get("ay_max", 12.0))

        for i in range(horizon):
            s_ref = s0 + i * self.mpc_dt * max(self.mpc_vx_min, 1.0)
            kappa = float(self.ref_lap.get_curvature(s_ref)[0])
            kappa_ref[i] = kappa
            if self.use_target_speed:
                vx_ref[i] = float(self.ref_lap.get_target_speed_value(s_ref))
            else:
                vx_ref[i] = min(TOP_SPEED_MS, np.sqrt(ay_max / max(abs(kappa), 1e-4)))
    ...
    def get_reference_trajectory(self, s0, horizon=None, dt=None, vx_current=None):
        horizon = horizon or self.mpc_horizon
        dt = dt or self.mpc_dt
        vx_current = float(self.mpc_vx_min if vx_current is None else max(vx_current, 0.0))
        ay_max = float(self.vehicle_params.get("ay_max", 12.0))
        wheelbase = float(self.vehicle_params["lf"] + self.vehicle_params["lr"])

        n_ref = len(self.ref_position_x_dist)
        s_samples = np.zeros(horizon + 1, dtype=np.float64)
        x_ref = np.zeros(horizon + 1, dtype=np.float64)
        y_ref = np.zeros(horizon + 1, dtype=np.float64)
        yaw_ref = np.zeros(horizon + 1, dtype=np.float64)
        kappa_ref = np.zeros(horizon + 1, dtype=np.float64)
        vx_ref = np.zeros(horizon + 1, dtype=np.float64)
        r_ref = np.zeros(horizon + 1, dtype=np.float64)
        delta_ref = np.zeros(horizon, dtype=np.float64)
        ax_ref = np.zeros(horizon, dtype=np.float64)

        s_cursor = float(s0)
        v_preview = max(vx_current, self.mpc_vx_min)
        for i in range(horizon + 1):
            ref_idx = int(np.clip(round(s_cursor), 0, n_ref - 1))
            kappa_i = float(self.ref_curvature_dist[ref_idx])
            if self.ref_target_speed_dist is not None:
                v_ref_i = float(self.ref_target_speed_dist[ref_idx])
            else:
                v_ref_i = min(TOP_SPEED_MS, np.sqrt(ay_max / max(abs(kappa_i), 1e-4)))

            s_samples[i] = s_cursor
            x_ref[i] = self.ref_position_x_dist[ref_idx]
            y_ref[i] = self.ref_position_y_dist[ref_idx]
            yaw_ref[i] = self.ref_yaw_dist[ref_idx]
            kappa_ref[i] = kappa_i
            vx_ref[i] = max(v_ref_i, self.mpc_vx_min)
            r_ref[i] = vx_ref[i] * kappa_i

            if i < horizon:
                delta_ref[i] = np.arctan(wheelbase * kappa_i)
                v_next_preview = max(vx_ref[i], self.mpc_vx_min)
                ax_ref[i] = np.clip((v_next_preview - v_preview) / max(dt, 1e-3),
                                    self.vehicle_params["ax_min"],
                                    self.vehicle_params["ax_max"])
                v_preview = v_next_preview
                s_cursor += dt * max(v_preview, self.mpc_vx_min)

        x_state_ref = np.column_stack([
            s_samples,
        ...
        u_ref = np.column_stack([delta_ref, ax_ref])
        return {
            "s": s_samples,
            "x": x_ref,
            "y": y_ref,
    ...
    def mpc_control_to_env_action(self, delta_cmd, ax_cmd):
        steer_max_rad = np.deg2rad(self.vehicle_params["steer_max_deg"])
        steer_norm = np.clip(delta_cmd / max(steer_max_rad, 1e-6), -1.0, 1.0)

        if ax_cmd >= 0.0:
            acc_level = np.clip(ax_cmd / max(self.vehicle_params["ax_max"], 1e-6), 0.0, 1.0)
            acc_norm = 2.0 * acc_level - 1.0
            brake_norm = -1.0
        else:
            brake_level = np.clip((-ax_cmd) / max(abs(self.vehicle_params["ax_min"]), 1e-6), 0.0, 1.0)
            acc_norm = -1.0
            brake_norm = 2.0 * brake_level - 1.0

        abs_action = np.array([steer_norm, acc_norm, brake_norm], dtype=np.float32)
        abs_action = np.clip(abs_action, self.controls_min_values, self.controls_max_values)

        if self.use_relative_actions:
            return self.inverse_preprocess_actions(self.current_actions.copy(), abs_action)
        return abs_action

    def solve_mpc(self, state=None):
        controller = self._build_mpc_controller()
        return controller.solve(self.get_curvilinear_state(state))

    def get_mpc_action(self, state=None):
        action, info = self.solve_mpc(state)
        self.mpc_last_action = action.astype(np.float32)
        return self.mpc_last_action.copy(), info

    def step_mpc(self):
        action, mpc_info = self.get_mpc_action()
    ...
    def preprocess_actions(self, actions, current_actions):
        if self.use_relative_actions:
            max_delta_action_high = self.controls_rate_limit[:, 1].astype(np.float32)
            current_actions += actions * max_delta_action_high
        else:
    ...
    def inverse_preprocess_actions(self, prev_abs_actions, current_abs_actions):
        if self.use_relative_actions:
            ...
            max_delta_action_high = self.controls_rate_limit[:, 1].astype(np.float32)
            actions = actions / max_delta_action_high
    ...
    def set_actions(self, actions):
        ...
        # msg = ",".join(map(str, actions))+ ","
        # data=msg.encode()
        # self.s.sendto(data,('192.168.43.148',2346))
    ...
    def step(self, action=None):
        ...
        if action is None and self.enable_mpc:
            action, _ = self.get_mpc_action()
        ...
        if self.config.screen_capture_enable:
            obs_image=self.get_current_image()
        ...
        return obs, self.state["reward"], bool(self.state["done"]),bool(self.state["done"]),buf_infos
    ...
    def expand_state(self, state):
        ...
        if  self.states:
            # diff=self.states[-1]['LapDist']-state["LapDist"] 
            # if diff>2000:
            #     self.total_laps = self.total_laps+1
            if self.prev_lapdist_raw is not None:
                diff = self.prev_lapdist_raw - state["LapDist"]
                if diff > 2000:
                    self.total_laps += 1
                    self.laps_since_reset += 1
        ...
        gap_left, _, _, _ = get_gap(closest_points, self.racing_left)
        gap_right, _, _, _ = get_gap(closest_points, self.racing_right)
        # closest point in the racing line
        state['rl_point'] = rl_point.item()

        state['gap'] = gap[0]
        state['gap_left'] = gap_left[0]
        state['gap_right'] = gap_right[0]
        ref_yaw = float(self.ref_lap.get_yaw(state["LapDist"]))
        ref_kappa = float(self.ref_lap.get_curvature(state["LapDist"])[0])
        state["s_mpc"] = state["LapDist"]
        state["ey_mpc"] = state["gap"]
        state["epsi_mpc"] = self._wrap_to_pi(state["yaw"] - ref_yaw)
        state["vx_mpc"] = state["local_velocity_x"]
        state["vy_mpc"] = state["local_velocity_y"]
        state["r_mpc"] = state["angular_velocity_y"]
        state["kappa_ref_mpc"] = ref_kappa
        ...
        if self.enable_out_of_track_termination and state['out_of_track']:##################################冲出来了
            ...
            segments=pd.DataFrame(self.states[-175:])
            lapdist = float(segments['LapDist'].iloc[-1])
            track_segment=self.track_left.get_track_segment(lapdist)
            self.failmode=self.ref_lap.diagnose_friction_failure(segments,track_segment)
        
        if self._check_custom_reset_condition(state["LapDist"]):
            logger.info(
                f"Terminate episode by custom LapDist reset: "
                f"phase={self.reset_phase_idx}, "
                f"target={self.reset_lapdist_points[self.reset_phase_idx]}, "
                f"laps_since_reset={self.laps_since_reset}, "
                f"LapDist={state['LapDist']:.2f}")
            done = 1
            # self.end_flag = True
            self.client.reset() 
            buf_infos['TimeLimit.truncated'] = True
            self._advance_reset_phase()
        ...
        if self.config.max_lapnum>0:
            if self.total_laps >= self.config.max_lapnum:
                logger.info(f"Terminate episode. Max steps {self.ep_steps}/{self.max_episode_steps}")
                done = 1
                self.end_flag=True
                buf_infos['TimeLimit.truncated'] = True
        ...
        if self.enable_low_speed_termination and state['speed'] < TERMINAL_LIMIT_PROGRESS_SPEED:
            ...
            if self.termination_counter <= 0:
                ...
                self.failmode = {
                    "failure_reason": "Speed Low",
                    "failure_LapDist": self.states[-1]["LapDist"],
                }
            # else:
        ...
        self.lapdist_fixed[self.ep_steps-1]=self.ref_lap.update_num(state["LapDist"])
        self.prev_lapdist_raw = state["LapDist"]

        buf_infos['type'] =self.lapdist_fixed
        buf_infos['LapDist']=state["LapDist"]
        state["done"] = done
    ...
    def get_reward(self, state, actions_diff):
        ...
        #轨迹线左侧 = gap 为-，右侧 = gap 为+
        # state['gap_left'] 参考轨迹点最近点 到左右边界的距离
        # state['gap_right'] 
        # print("!!!!!!!!!!!!!!!!!!!!!!!!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        # print("left")
        # print(state['gap_left'])
        # print(state['gap_right'])
        if np.abs(state['gap_left'])<gap_threshold:
            # state["gap"]=state["gap"]-(gap_threshold-np.abs(state['gap_left']))
            gap_threshold=np.abs(state['gap_left'])
        elif np.abs(state['gap_right'])<gap_threshold:
            # state["gap"]=state["gap"]+(gap_threshold-np.abs(state['gap_right']))
            gap_threshold=np.abs(state['gap_right'])

        if np.abs(state["gap"]) > gap_threshold:
            # Apply the penalty when the gap exceeds the threshold
            r = 3.6 * state['speed'] * (1.0 - ((np.abs(state["gap"]) - gap_threshold) / 12.00))
        else:
            # No gap penalty, just the speed reward
            r = 3.6 * state['speed']
    ...
    def reset(self, seed=None, options=None):
        ...
        self.laps_since_reset = 0
        self.prev_lapdist_raw = None
        self.mpc_last_action = self.start_actions.copy()
        if self.mpc_controller is not None:
            self.mpc_controller.last_u_sequence[:] = 0.0
            self.mpc_controller.last_solution[:] = 0.0

        # flush a few steps. Otherwise the history state will be incomplete
        for _ in range(2):
            obs, _, _, _, _ = self.step(self.start_actions)##############

        self.states = []
        obs, _, _, _, info = self.step(self.start_actions)################
        self.lapdist_fixed = self.lapdist_fixed = np.full(self.max_episode_steps, -1, dtype=np.float32)
        self.ref_lap.reset_num(info['LapDist'])
    ...
    def end_of_episode_stats(self, verbose=True):
        ...
        # self.lapdist_fixed[self.ep_steps-1]
        ...
        if len(self.states) and self.stats_saved is False:
            ...
            if self.save_observations:
                ...
                if save_csv:
                    save_csv_channels = ['steps', 'currentTime', 'done',
                                         ...
                                         'tyre_slip_ratio_fl', 'tyre_slip_ratio_fr', 'tyre_slip_ratio_rl', 'tyre_slip_ratio_rr',
                                         'SlipAngle_fl', 'SlipAngle_fr', 'SlipAngle_rl', 'SlipAngle_rr',
                                         'fl_wheel_load', 'fr_wheel_load', 'rl_wheel_load', 'rr_wheel_load',
            ...
            if self.failmode is not None:
                r["failure_reason"] = self.failmode ["failure_reason"]
                r["failure_LapDist"] = self.failmode ["failure_LapDist"]
            else:
                r["failure_reason"] = "Time Up"
                r["failure_LapDist"] = np.nan
...
register(
    id="AssettoCorsaEnv",
    entry_point="assetto_corsa_gym.AssettoCorsaEnv.ac_env:AssettoCorsaEnv",
    # max_episode_steps=15000,
)
