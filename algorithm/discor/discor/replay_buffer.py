...
class ReplayBuffer:
    ...
    def export_lastN(self, keep_last=10_000):
        n = min(self._n, keep_last)
        # 计算最后 n 条的索引（连续数组，所以简单切片即可）
        idxs = np.arange(self._n - n, self._n)

        return dict(
            states=self._states[idxs].copy(),
...
class MixBuffer:
    """
    buffers: [buf_1, buf_2, ..., buf_K]
    weights: 对应权重，可以是不归一化的正数；内部会归一化为占比。
    temperature: 温度系数，>1 更平均，<1 更尖锐（可做 curriculum/anneal）
    """
    def __init__(self, buffers, weights, temperature: float = 1.0):
        assert len(buffers) == len(weights) and len(buffers) > 0
    ...
    def _proportions(self):
        w = np.clip(self.weights, 1e-12, None) ** (1.0 / self.temperature)
        w = w / w.sum()
        return w
    ...
    def sample(self, batch_size: int, device):
        p = self._proportions()
        # 多项分布决定各 buffer 的配额
        counts = np.random.multinomial(batch_size, p)

        parts = []
        for buf, n in zip(self.buffers, counts):
            if n > 0:
                parts.append(buf.sample(n, device))

        # 聚合 & 打乱
        batch = [torch.cat([b[i] for b in parts], dim=0) for i in range(5)]
        perm = torch.randperm(batch[0].shape[0], device=device)
        return tuple(x[perm] for x in batch)
